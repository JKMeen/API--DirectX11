...
bool ModelClass::Initialize(int m_vCount, int m_iCount, float radius, int nPoints, ID3D11Device* device)
{
	bool result;
...
void ModelClass::Set_Position(vec3d loc){
	for (int i = 0; i < 3; i++){
		...
		if (this->Model->location->v[i] <= -50)
			this->Model->location->v[i] = 50;
		if (this->Model->location->v[i] >= 50)
			this->Model->location->v[i] = -50;
	}
}
